...
def Local_score(date):
    ...
    for stockiid in All:
        cur = db.cursor()
        cur.execute("""select `date`, `local_company` from margin_trading_short_selling 
                        where stockiid={} 
        ...
        for data in Data:
            if data[1] <0:
                judge+=1
        if judge > 0:
            r.hset('STOCK_{}'.format(stockiid), 'local3', 0)
            print('redis_local', stockiid,0)
        else:
            r.hset('STOCK_{}'.format(stockiid), 'local3', 1)
            print('redis_local', stockiid,1)
    return date
