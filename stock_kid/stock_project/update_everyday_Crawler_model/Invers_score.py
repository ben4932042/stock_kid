...
def Invers_score(date):
    ...
    for stockiid in All:
        cur = db.cursor()
        cur.execute("""select `date`, `investment_bank` from margin_trading_short_selling 
                        where stockiid={} 
        ...
        for data in Data:
            if data[1] <0:
                judge+=1
        if judge > 0:
            r.hset('STOCK_{}'.format(stockiid), 'inves3', 0)
            print('redis_invers', stockiid,0)
        else:
            r.hset('STOCK_{}'.format(stockiid), 'inves3', 1)
            print('redis_invers', stockiid,1)
    return date
